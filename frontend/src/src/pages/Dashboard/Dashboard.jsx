      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400">
            ({Math.round(totalBurned)} cal this week)
          </span>
        </div>
        <div className="flex items-center gap-4 text-xs">
          <span className="text-[10px] text-gray-500 mr-2">
            Val: [{chartData.map((d) => d.Burned).join(", ")}]
          </span>
...
const StreakHeatmap = ({ activityData, darkMode = false }) => {
  const days = ["M", "T", "W", "T", "F", "S", "S"];
  const mutedTextClass = "text-gray-100";
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {days.map((day, i) => (
          <div
            key={i}
            className={`text-center text-xs font-medium ${mutedTextClass}`}
          >
            {day}
          </div>
        ))}
      ...
      <div className="flex items-center justify-end gap-4 mt-3 text-xs">
        <div className="flex items-center gap-1">
          ...
          <span className={mutedTextClass}>Rest</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-[#84CC16]" />
          <span className={mutedTextClass}>Active</span>
        </div>
      </div>
    </div>
  );
...
// BMI Trend Chart
const BMITrendChart = ({ bmiHistory, darkMode = false }) => {
  ...
  const valueTextClass = "text-white";
  const dateTextClass = "text-gray-100";
...
export default function Dashboard() {
    ...
    try {
      const [s, a, b, w, c, p, l] = await Promise.all([
        ...
        axios
          ...
          .catch(() => ({ data: { plan: null, sessionLogs: [] } })),
        axios
          .get(`${API_BASE_URL}/api/auth/calorie-intake/history/${user._id}`, {
            params: { _t: Date.now() },
          })
          .catch(() => ({ data: { logs: [] } })),
        axios
          ...
          .catch(() => ({ data: { sessions: [] } })),
        axios
          .get(`${API_BASE_URL}${API_ENDPOINTS.GAMIFY}/leaderboard`, {
            params: { period: "all", _t: Date.now() },
          })
          .catch(() => ({ data: { users: [] } })),
      ]);
  ...
  const weeklyExerciseData = useMemo(() => {
    ...
    return dataSeconds.map((secs) =>
      secs > 0 ? Math.max(1, Math.round(secs / 60)) : 0,
    );
  ...
  const getAIInsight = () => {
    if (stats.streakCount >= 7)
      return {
        ...
        text: `Your ${stats.streakCount}-day streak shows remarkable dedication. You're in the top 10% of consistent users!`,
      };
    if (adherence.adherenceThisWeek < 50)
      return {
        title: "Optimize Rest Days",
        text: "Your calorie intake on rest days was 7% above goal. Consider reducing carb intake on rest days to maintain a deficit.",
      };
    if (stats.streakCount >= 3)
      return {
        title: "Building Habits",
        text: "You're developing a solid routine! 3 more days to unlock the 7-day streak badge.",
      };
    return {
      title: "Start Strong",
      text: "Begin your fitness journey today! Even 15 minutes of activity can boost your metabolism by 10%.",
    };
  };
  ...
  if (loading) {
    return (
      <div
        className={`min-h-screen flex flex-col transition-colors duration-300 ${darkMode ? "bg-[#0A0F1C] text-white" : "bg-gray-50 text-gray-900"}`}
      >
        <NavBar />
      ...
      {stats?.weeklyChallenge?.title && (
        <div className="bg-gradient-to-r from-[#22D3EE] via-[#8B5CF6] to-[#A855F7] text-white p-3 font-bold text-center text-sm z-40 shadow-lg border-b border-[#22D3EE]/30 relative overflow-hidden group">
          <div className="flex items-center justify-center gap-3">
            <span className="flex items-center justify-center bg-white/20 p-1.5 rounded-lg animate-pulse">
              🏆
            </span>
            <span className="truncate">
              <span className="text-[#FACC15]">Active Weekly Challenge:</span>{" "}
              {stats.weeklyChallenge.title} —{" "}
              {stats.weeklyChallenge.progress || 0}/
              {stats.weeklyChallenge.target || 3} workouts!
            </span>
            {stats.weeklyChallenge.weekEndAt && (
              <span className="text-xs bg-black/30 border border-white/20 px-3 py-1 rounded-full shrink-0">
                Ends:{" "}
                {new Date(stats.weeklyChallenge.weekEndAt).toLocaleDateString()}
              </span>
            )}
          </div>
        </div>
        ...
        <section className="py-6 sm:py-8">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#84CC16]/10 border border-[#84CC16]/30">
                  ...
                  <span className="text-sm text-[#84CC16] font-medium">
                    AI-Powered
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              {/* Calories Burned */}
              <div
                className={`relative rounded-2xl p-5 transition-all group border ${darkMode ? "bg-[#111827] border-[#1F2937]" : "bg-white border-gray-200 shadow-sm text-gray-900"}`}
              >
              ...
              >
                ...
                <p className="text-sm text-gray-100">Consistency Score</p>
                <p className="text-xs mt-1 text-gray-100">Last 4 weeks</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <div
                className={`lg:col-span-2 rounded-2xl p-6 border ${darkMode ? "bg-[#111827] border-[#1F2937]" : "bg-white border-gray-200 shadow-sm text-gray-900"}`}
              >
                <WeeklyBarChart
              ...
              {/* Weekly Goal */}
              <div
                className={`rounded-2xl p-6 border ${darkMode ? "bg-[#111827] border-[#1F2937]" : "bg-white border-gray-200 shadow-sm text-gray-900"}`}
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div
                    className={`text-center p-3 rounded-xl border ${darkMode ? "bg-[#1F2937]/50 border-[#374151]" : "bg-gray-50 border-gray-200"}`}
                  >
                    <div className="text-xl font-bold text-white">
                      {Math.round(
                        weeklyExerciseData.reduce((a, b) => a + b, 0),
                      )}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              >
                <StreakHeatmap
                  activityData={streakHeatmapData}
                  darkMode={darkMode}
                />
              </div>
            </div>
            ...
            {/* AI Insights Section */}
            <div
              className={`rounded-2xl p-6 mb-6 border ${darkMode ? "bg-[#111827] border-[#1F2937]" : "bg-white border-gray-200 shadow-sm text-gray-900"}`}
            >
              <div className="flex items-center gap-2 mb-4">
              ...
              <div
                className={`p-4 rounded-xl border ${darkMode ? "bg-[#1F2937]/30 border-[#374151]/50" : "bg-gray-50 border-gray-200 text-gray-900"}`}
              >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-400 capitalize">
                        Goal:{" "}
                        {workoutPlan.generatedParams?.fitnessGoal?.replace(
                          /_/g,
                          " ",
                        ) || "N/A"}
                      </p>
                    ...
                    <div className="flex-1 max-w-xs w-full">
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-[#3B82F6] font-semibold">
                          {workoutPlan.completedDayCount || 0} days
                        </span>
                      </div>
                      <div className="h-2 rounded-full bg-gray-700 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, ((workoutPlan.completedDayCount || 0) / (workoutPlan.generatedParams?.daysPerWeek * workoutPlan.durationWeeks || 28)) * 100)}%`,
                          }}
                        />
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              >
                {bmiHistory.length > 0 ? (
                  <BMITrendChart bmiHistory={bmiHistory} darkMode={darkMode} />
                ) : (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2 mb-4 p-3 rounded-xl bg-[#1F2937]/30 border border-[#374151]/30">
                  ...
                  {leaderboardRank > 0 && leaderboardRank < 10 && (
                    <GamifyBadge type="top10" />
                  )}
                  {leaderboardRank >= 10 && leaderboardRank < 50 && (
                    <GamifyBadge type="top50" />
                  )}
                  {(stats.streakCount || 0) >= 7 && (
                    <GamifyBadge type="beast" />
                  )}
                  {!(leaderboardRank >= 0 && leaderboardRank < 50) &&
                    !((stats.streakCount || 0) >= 7) && (
                      <span className="text-xs text-gray-400">
                        No leaderboard badges yet
                      </span>
                    )}
                </div>
                ...
                {(stats.badges || []).length > 0 ? (
                  <div className="space-y-2">
                    {(stats.badges || []).slice(0, 3).map((badge, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 p-3 rounded-lg bg-[#1F2937]/50 border border-[#FACC15]/20"
                      >
